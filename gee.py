...
def summary(task_id, detailed=True, props=None):
    ...
    if detailed:
        ...
        if tupdate:
            if tstart:
                run_time = timedelta(milliseconds=tupdate - tstart)
            else:
                run_time = "NOT RUNNING"
            if tcreate:
                total_time = timedelta(milliseconds=tupdate - tcreate)
            else:
                total_time = "NOT CREATED"
        else:
            run_time = "NO DATA"
            total_time = "NO DATA"
